<?php
    ...
    if(($account['role_id'] == 1) || ($account['role_id'] == 4)){ //wyrzuć gdy nie masz uprawnień
        header('Location: index.php');
    }
    ...
    if(isset($_POST['title']) && isset($_POST['price']) && isset($_POST['description']))//Jeśli wyslal formularz to wstaw do bazy danych i przekieruj do logowania
    {
        ...
        $result = $funkcje->addProduct($_POST['title'], $_POST['description'], $_POST['price'], $image_number .".". $whatIWant, 'ebook', $_POST['product_origin'], $_POST['product_category']);//Jesli true to znaczy, ze zarejestrowalo(sprawdz return w metodzie register w Funkcje.php)
        if($result)
        {
...
<html lang="pl">
    ...
    <body>
        ...
        <div class="login-register-page">  
            <h2 class="fade-in-text">Dodawanie ebooka</h2>
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="text_field mt-1">
                    <label for="text">Rodzaj kuchni</label><br>
                    <!-- <label for="text">Tylko w wypadku dodania ebooka</label><br> -->
                    <select name="product_category"> 
                        <?php 
                            $categories = $funkcje->getCategories();
                            foreach($categories as $category){
                                echo "<option value='". $category['category_number'] ."'>". $category['category_name'] ."</option>";
                            }
